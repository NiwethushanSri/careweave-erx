    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-5">
        {[
          ...
          { label: 'Dispensed Today', count: counts.dispensed, icon: CheckCircle, color: 'text-green-500' },
        ].map(s => (
            ...
            {filtered.map(p => (
              <div key={p.id}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-700">{p.patient_name}</p>
                ...
                <div className="flex items-center gap-2 ml-4">
                  <p className="text-xs text-gray-400 mr-2">{format(new Date(p.created_at), 'dd MMM')}</p>
                  {p.status === 'sent' && (
                    <button onClick={(e) => handleReceive(p.id, e)} className="btn-secondary text-xs py-1.5 px-3">
                      Receive
                    </button>
                  )}
                  {p.status === 'received' && (
                    <button onClick={(e) => handleDispense(p.id, e)} className="btn-primary text-xs py-1.5 px-3">
                      Dispense
                    </button>
                  )}
                </div>
